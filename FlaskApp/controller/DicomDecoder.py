...
class DicomDecoder:
    ...
    def readPixels(self): #only 16-bit Dicom files are supported
        if(self.samplesPerPixel == 1 and self.bitsAllocated == 16): #16bit grayscale
            self.pixels16 = []
            self.pixels16Int = []
            ...
            for i in range(numPixels*2):
                bufByte.append(self.getByte())

            for i in range(numPixels):
                i1 = i*2
                b0 = bufByte[i1]
                b1 = bufByte[i1 + 1]
                unsignedS = int((b1 << 8) + b0)
                if (self.pixelRepresentation == 0): #unsigned
                    pixVal = int(unsignedS * self.rescaleSlope + self.rescaleIntercept)
                    if (self.photoInterpretation == "MONOCHROME1"):
                        pixVal = self.max16 - pixVal
                else:
                    #convert rawData to Singned Short
                    sVal = self.getSignedShort(unsignedS)
                    # Need to consider rescale slope and intercepts to compute the final pixel value
                    pixVal = int(sVal * self.rescaleSlope + self.rescaleIntercept)
                    if (self.photoInterpretation == "MONOCHROME1"):
                        pixVal = self.max16 - pixVal
                self.pixels16Int.append(pixVal)
            minVal = min(self.pixels16Int)

            self.signedImage = False
            if (minVal < 0):
                self.signedImage = True;
            for pixel in self.pixels16Int:
                if (self.signedImage):
                    self.pixels16.append(int(pixel + 0x7FFF))
                else:
                    self.pixels16.append(int(pixel))
            self.pixels16Int = []
